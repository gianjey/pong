...
//if return value is 1 collision occured. if return is 0, non collision.
...
static void move_ball()
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		if (c == 1)
		{
			...
			if (ball.dx > 0)
			{
				//teleport ball to avoid mutli collison glitch
				if (ball.x < 30)
				{
					ball.x = 30;
				}
...
static void move_paddle_ai()
{
	...
	int screen_cemter = screen->h / 2 - 25;
...
static void draw_player_score(int player)
{
	SDL_Rect src;
	SDL_Rect dest;
...
int main(int argc, char* args[])
{
	...
	while (quit == 0)
	{
		...
		else if (state == 2)
		{
			...
			{
				state = 0;
				//delay for a little bit so the space bar press doesnt get triggered twice
...
int init(int width, int height, int argc, char* args[])
{
	...
	//create the screen surface where all the elements will be draw onto (ball, paddles, net etc)
	...
	{
		printf("Could not create the screen_textture! SDL_Error: %s\n", SDL_GetError());
		return 1;
	}
